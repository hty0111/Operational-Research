...
def run(work_man, index, work_days, A_times, B_times, C_times, D_times, qualify, s):
    # 员工人数
    count = len(index)
    # 已经安排班次的次数
        ...
        if count>=8:
            ...
            if count>=11:
                ...
                if count>=13:
                    obj += lpDot(D[i],tmp[3])
    obj2 = ''
    # 目标函数第二部分，尽量避免前一天工作的人员第二天再安排 
    for i in range(len(work_man)):
        obj2 += lpDot(A[work_man[i]],1)
        if count>=8:
            obj2 += lpDot(B[work_man[i]],1)
            if count>=11:
                obj2 += lpDot(C[work_man[i]],1)
                if count>=13:
                    obj2 += lpDot(D[work_man[i]],1)
    ...
    # 目标函数第三部分，每个人的工作安排均衡
    if count <= 7:
        my += lambda1*obj + lambda2*obj2 + lpDot(A,work_rate)
    elif 8 <= count <= 10:
        my += lambda1*obj + lambda2*obj2 + lpDot(A,work_rate)+lpDot(B,work_rate)
    elif 11 <= count <= 12:
        my += lambda1*obj + lambda2*obj2 + lpDot(A,work_rate)+lpDot(B,work_rate)+lpDot(C,work_rate)
    elif 13 <= count <= 15:
        my += lambda1*obj + lambda2*obj2 + lpDot(A,work_rate)+lpDot(B,work_rate)+lpDot(C,work_rate)+lpDot(D,work_rate)
    ...
    # work_man用于记录上一次工作的人员序号
    work_man = []
    # solution用于记录A,B,C,D四个0-1规划数组
    solution = []
...
while(1):
    ...
    # 因为求解规划时0-1变量的下表会变，需要不断更新work_man数组
    new_work_man = []
    for i in range(len(index)):
        if index[i] in work_man:
            new_work_man.append(i)
    ...
    # get solution
    # 第一问
    solution, objective, work_man = run(new_work_man, index, work_days_tmp, A_times_tmp, B_times_tmp, 
